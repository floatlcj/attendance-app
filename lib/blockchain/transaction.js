...
class Transaction {
    ...
    check() {
        ...
        if (this.type == 'registration') {
            // Check if the signature of the registration is correct (public key signed by the secret key)
            let isValidSignature = CryptoEdDSAUtil.verifySignature(this.data.publicKey, this.data.signature, CryptoUtil.hash({
                publicKey: this.data.publicKey,
                stuid: this.data.stuid
            }));

            if (!isValidSignature) {
                console.error(`Invalid transaction registration signature '${JSON.stringify(this.data)}'`);
                throw new TransactionAssertionError(`Invalid transaction registration signature '${JSON.stringify(this.data)}'`, this.data);
            }
        }

        if (this.type == 'attendance') {
            // Check if the signature of the attendance is correct (public key signed by the secret key)
            let isValidSignature = CryptoEdDSAUtil.verifySignature(this.data.publicKey, this.data.signature, CryptoUtil.hash({
                stuid: this.data.stuid,
            ...
            if (!isValidSignature) {
                console.error(`Invalid transaction attendance signature '${JSON.stringify(this.data)}'`);
                throw new TransactionAssertionError(`Invalid transaction attendance signature '${JSON.stringify(this.data)}'`, this.data);
            }
        }
        ...
        if (this.type == 'regular' || this.type == 'attendance' || this.type == 'registration') {
