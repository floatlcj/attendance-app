...
class Blockchain {
    ...
    getRegistrationTransactionByStuid(stuid) {
        // Find the registration transaction for a student id
        // return R.find(
        //     R.compose(
        //         R.find(
        //             R.both(
        //                 R.propEq('type', 'registration'),
        //                 R.pathEq(['data', 'stuid'], stuid)
        //             )
        //         ),
        //         R.prop('transactions')
        //     ),
    ...
    checkTransaction(transaction, referenceBlockchain = this.blocks) {
        ...
        if (transaction.type === 'attendance') {
            ...
            let registration = this.getRegistrationTransactionByStuid(transaction.data.stuid);
            if (!registration) {
                console.error(`No registration found for student id '${transaction.data.stuid}'`);
                throw new TransactionAssertionError(`No registration found for student id '${transaction.data.stuid}'`, transaction.data.stuid);
            }
            let regHash = CryptoUtil.hash({
                stuid: transaction.data.stuid,
            ...
            if (!isValidSignature) {
                ...
                throw new TransactionAssertionError(`Invalid transaction registration signature '${JSON.stringify(transaction.data)}'`, transaction.data);
            }
        }
    ...
    getAttendanceListForId(stuid) {
        let registraction = this.getRegistrationTransactionByStuid(stuid);
        if (registraction === null) {
            throw new ArgumentError(`No registration found for student id '${stuid}'`);
        }
        return this.getAttendanceListForAddress(registraction.data.publicKey);
    }

    getAttendanceListForAddress(address) {
        ...
        let attendanceList = [];
        R.forEach(R.pipe(R.prop('transactions'), selectTxs), this.blocks);
